...
// TestAuthAPIWithSnapshot は Auth API のスナップショットテストを実行します
func TestAuthAPIWithSnapshot(t *testing.T) {
	fmt.Println("🚀 Starting SaaSus Auth API Snapshot Tests")
	fmt.Println("==============================================")
	...
	engine := snapshot.NewE2EEngineWithSnapshot(client, GetAuthMethods(), config)
...
// parseSnapshotFlags はコマンドラインフラグを解析してスナップショットモードを設定します
